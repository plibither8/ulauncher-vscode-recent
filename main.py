...
class Utils:
    ...
    def get_path(filename, from_home = False):
        base_dir = pathlib.Path.home() if from_home else pathlib.Path(__file__).parent.absolute()
...
class Code:
    ...
    def get_recents(self):
        recents = []
        storage = json.load(open(Utils.get_path(".config/Code/storage.json", True), "r"))
...
class CodeExtension(Extension):
    ...
    def get_ext_result_items(self, query):
        ...
        data = list(
            filter(
                lambda c: c["label"].lower().startswith(query),
                recents,
            )
        )
        for recent in data[0:20]:
            items.append(
                ExtensionSmallResultItem(
                    icon=Utils.get_path(f"images/{'folder' if recent['folder'] else 'file'}.svg"),
                    name=recent["label"],
