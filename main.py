...
class Code:
    ...
    def __init__(self):
        ...
        for path in (pathlib.Path(path_dir) for path_dir in Code.path_dirs):
            for variant in Code.variants:
                ...
                logger.debug(
                    'evaluating installation dir %s and config dir %s', installed_path, config_path)
                if installed_path.exists() and config_path.exists() and (config_path / "User" / "globalStorage" / "storage.json").exists():
                    logger.debug(
                        'found installation dir %s and config dir %s', installed_path, config_path)
        ...
        logger.warning(
            'Unable to find VS Code installation and config directory')
    ...
    def get_recents(self):
        ...
        if self.global_state_db.exists():
            ...
            try:
                ...
                logger.error(
                    'getting recents from global state database failed', e)
    ...
    def get_recents_global_state(self):
        logger.debug('connecting to global state database %s',
...
class CodeExtension(Extension):
    ...
    def get_ext_result_items(self, query):
        ...
        for match in uri_matches:
            ...
            existing = next(
                (c for c in data if c["uri"] == recent["uri"]), None)
